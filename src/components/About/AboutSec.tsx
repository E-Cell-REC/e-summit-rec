...
const values: Value[] = [
  { title: "Describe value one", desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros elementum tristique. Duis cursus, mi quis viverra ornare, eros dolor interdum nulla." },
  { title: "Describe value two", desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros elementum tristique. Duis cursus, mi quis viverra ornare, eros dolor interdum nulla." },
  { title: "Describe value three", desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros elementum tristique. Duis cursus, mi quis viverra ornare, eros dolor interdum nulla." },
];

const AboutSec: React.FC = () => {
  return (
    <div className="aboutSection text-white bg-gray-900 px-6 md:px-20">
      {/* Header Section */}
      <div className="headSec text-white h-auto md:h-[25vh] w-full mb-10 text-center md:text-left">
          ...
          <div className="statsSec grid grid-cols-2 gap-4 mt-4">
            {stats.map((stat, index) => (
              <div key={index} className="box text-center">
                <h2 className="text-2xl md:text-3xl font-bold">{stat.number}</h2>
                <p>{stat.label}</p>
              </div>
            ))}
          </div>
      ...
      {/* Image Section */}
      <div className="imgSec text-white h-[50vh] md:h-[80vh] mt-4">
        <img 
          src={aboutPlace} 
      ...
      <div className="pointsSec text-white h-auto md:h-[40vh] mt-4 flex flex-wrap gap-10">
        {values.map((value, index) => (
          <div key={index} className="pointBox w-full md:w-[30%]">
            <img className="mb-6" src={pointsIcon} alt={`Icon for ${value.title}`} />
            <h3 className="text-lg md:text-xl font-bold mb-3 md:mb-6">{value.title}</h3>
            <p>{value.desc}</p>
          </div>
        ))}
      </div>
